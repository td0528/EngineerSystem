...
async def update_inbound_sap_status(
    ...
    if stock:
        ...
        convertible_qty = min((stock.unrecorded_sap_qty or 0), qty)
        transfer_withdraw = min((stock.unrecorded_withdrawn_qty or 0), convertible_qty)
        transfer_sap = max(0, convertible_qty - transfer_withdraw)

        stock.unrecorded_sap_qty = max(0, (stock.unrecorded_sap_qty or 0) - convertible_qty)
        ...
        if transfer_sap > 0:
            stock.sap_qty = (stock.sap_qty or 0) + transfer_sap
...
@router.put("/outbound/{record_id}/receipt-done")
async def mark_outbound_receipt_done(record_id: int, db: Session = Depends(get_db)):
    """将出库记录收货单状态标记为已做"""
    ...
    outbound.has_receipt = "是"
    db.commit()
    return {"success": True, "message": "已标记为已做收货单", "has_receipt": outbound.has_receipt}
