...
async def get_received_inbound(
    ...
    if search:
        ...
        query = query.filter(
            or_(
                ...
                Material.name_cn.ilike(term),
                Material.name_vn.ilike(term),
                PurchaseOrder.supplier.has(name=PurchaseOrder.supplier.property.mapper.class_.name.ilike(term))
            )
        )
    ...
    if supplier_name:
        query = query.filter(PurchaseOrder.supplier.has(name == supplier_name))
